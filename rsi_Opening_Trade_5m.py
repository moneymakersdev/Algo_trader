...
while True:
    ...
    if str(datetime.datetime.now()) >= str(data.date())+" 09:30:00.000000":
        ...
        while True:
            ...
            sql = "select * from rsi_opening_trade_5m ORDER BY DateTime DESC"
            ...
            if Position == "0":
                ...
                if float(Idx_Live) > float(High_15):
                    ...
                    if float(Idx_Live) > float(MA_New):                    
                        trade = "INSERT INTO rsi_opening_trade_5m (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL, Profit, TSL) values ('"+str(symbol.replace("'", ""))+"', '"+str(tokenresult.replace("'", ""))+"', 'BUY', '30', '"+str(option_live)+"', '"+str(Idx_Live)+"', '1', '"+str(datetime.datetime.now())+"', 'Rsi_Trades_5m', 'MA', 'None', '"+str(float(option_live)+40)+"')"
                        mycursor.execute(trade)
                        mydb.commit()
                    else:
                        trade = "INSERT INTO rsi_opening_trade_5m (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL, Profit, TSL) values ('"+str(symbol.replace("'", ""))+"', '"+str(tokenresult.replace("'", ""))+"', 'BUY', '30', '"+str(option_live)+"', '"+str(Idx_Live)+"', '1', '"+str(datetime.datetime.now())+"', 'Rsi_Trades_5m', 'None', 'MA', '"+str(float(option_live)+40)+"')"
                        mycursor.execute(trade)
                        mydb.commit()


                    data = "{'TSL': '0', 'Target': '"+str(float(option_live)+40)+"'}"
                    with open('rsi_opening_trade_5m.json', 'w') as outfile:
                        outfile.write(data)
                        outfile.close()
            ...
            if Position == "1":
                ...
                with open('rsi_opening_trade_5m.json', 'r') as outfile:
                   data = outfile.read()
                ...
                if float(Option_live) <= float(Option_Price)-30:
                    ...
                    trade = "INSERT INTO rsi_opening_trade_5m (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL, Profit, TSL) values ('"+str(Symbol.replace("'", ""))+"', '"+str(Token.replace("'", ""))+"', 'SL', '30', '"+str(Option_live)+"', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'Rsi_Trades_5m', '0', '0', '0')"
                    mycursor.execute(trade)
                    mydb.commit()
                    exit()
                ...
                if str(datetime.datetime.now()) >= str(data.date())+" 15:27:00.000000":
                    ...
                    Sell_Order(Symbol, Token, "Rsi_Trades_5m")
                    trade = "INSERT INTO rsi_opening_trade_5m (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL, Profit, TSL) values ('"+str(Symbol.replace("'", ""))+"', '"+str(Token.replace("'", ""))+"', 'SELL', '30', '"+str(Option_live)+"', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'Rsi_Trades_5m', '0', '0', '0')"
                    mycursor.execute(trade)
                    mydb.commit()
