...
while True:
    ...
    if str(datetime.datetime.now()) > str(data.date())+" 09:30:00.000000":
        ...
        while True:
            ...
            apikey = "YhzUtemX"
            secretkey = "7c677165-6b94-43cd-90ae-704a76d48dab"
            user_id = "N50732994"
            password = "3333"
            totp = "XWXQENYUKZF46RN5VMFIH25BYU"
            ...
            sql = "select * from delta_finnifty_fut_orders_1h ORDER BY DateTime DESC"
            ...
            if Position == "0":
                ...
                if float(Latest) > float(Supertrend_0_4) and float(Latest) > float(Supertrend_0_8) and float(Latest) > float(ema1) and float(Previous) < float(Supertrend_0_4):# and float(Previous) < float(Supertrend_0_8) and float(Previous) < float(ema1):
                    ...
                    DhanSymbol = "FINNIFTY "+" "+str(ExpiryMonth)+" "+" FUT"
                    AliceSymbol = "FINNIFTY "+str(ExpiryMonth)+" "+" FUT"
                    UpstoxSymbol = "FINNIFTY"+str(ExpiryYear)+str(ExpiryMonth)+"FUT"
                    FyresSymbol = "FINNIFTY"+str(ExpiryYear)+str(ExpiryMonth)+"FUT"
                    ...
                    Buy_Order(str(symbol.replace("'", "")), str(tokenresult[0].replace("'", "")), "VPS_FIN_Future", DhanSymbol, AliceSymbol, UpstoxSymbol, FyresSymbol, MotilalSymbol)

                    trade = "INSERT INTO delta_finnifty_fut_orders_1h (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('FINNIFTY_FUTURE', 'None', 'BUY', '30', '0', '"+str(Idx_Live)+"', '1', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                ...
                if float(Latest) < float(Supertrend_0_4) and float(Latest) < float(Supertrend_0_8) and float(Latest) < float(ema1) and float(Previous) > float(Supertrend_0_4): #and float(Previous) > float(Supertrend_0_8) and float(Previous) > float(ema1):
                    ...
                    Short_Order(str(symbol.replace("'", "")), str(tokenresult[0].replace("'", "")), "VPS_FIN_Future", DhanSymbol, AliceSymbol, UpstoxSymbol, FyresSymbol, MotilalSymbol)

                    trade = "INSERT INTO delta_finnifty_fut_orders_1h (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('FINNIFTY_FUTURE', 'None', 'SHORT', '30', '0', '"+str(Idx_Live)+"', '1', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                    mycursor.execute(trade)
                    mydb.commit()
            ...
            if Position == "1":
                ...
                if Type == "BUY":
                    ...
                    if float(Latest) < float(Supertrend_0_4) and float(Latest) < float(Supertrend_0_8) and float(Latest) < float(ema1):
                        ...
                        Sell_Order(Symbol, Token, "VPS_FIN_Future")

                        trade = "INSERT INTO delta_finnifty_fut_orders_1h (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('FINNIFTY_FUTURE', 'None', 'SELL', '30', 'None', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
                ...
                if Type == "SHORT":
                    ...
                    if float(Latest) > float(Supertrend_0_4) and float(Latest) > float(Supertrend_0_8) and float(Latest) > float(ema1):
                        ...
                        Buy_Order(str(symbol.replace("'", "")), str(tokenresult[0].replace("'", "")), "VPS_FIN_Future", DhanSymbol, AliceSymbol, UpstoxSymbol, FyresSymbol, "None")

                        trade = "INSERT INTO delta_finnifty_fut_orders_1h (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('FINNIFTY_FUTURE', 'None', 'SHORTEXIT', '30', 'None', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
