...
def Zrodha_Buy_Order_NRML(Api, Api_Secret, Totp, symbol, token, strategy, userid, producttype, quantity):
    api_key = Api
    Secret = Api_Secret
...
def Zerodha_Sell_Order_NRML(Api, Api_Secret, Totp, symbol, token, strategy, userid, producttype, quantity):
    api_key = Api
    Secret = Api_Secret
    ...
    try:
        ...
        trade = "INSERT INTO zerodha_orders (Symbol, OrderID, Price, Quantity, UserID, Type, DateTime, Strategy) values ('"+str(symbol)+"', '"+str(orderId)+"', '"+str(Price)+"', '"+str(quantity)+"', '"+str(userid)+"', 'SELL', '"+str(datetime.datetime.now())+"', '"+str(strategy)+"')"
        mycursor.execute(trade)
        mydb.commit()
