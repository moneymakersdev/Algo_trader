...
while True:
    ...
    if str(datetime.datetime.now()) > str(data.date())+" 11:15:00.000000":
        ...
        while True:
            ...
            GetExpiry_Date = Midcp_Nifty_Monthly_Expiry()
            ...
            sql = "select * from delta_midcp_fut_orders_1h ORDER BY DateTime DESC"
            ...
            if Position == "0":
                ...
                if float(Latest) > float(Supertrend_0_4) and float(Latest) > float(Supertrend_0_8) and float(Latest) > float(ema1) and float(Previous) < float(Supertrend_0_4):# and float(Previous) < float(Supertrend_0_8) and float(Previous) < float(ema1):
                    symbol = "MIDCPNIFTY"+str(ExpiryDate)+str(ExpiryMonth)+str(ExpiryYear)+"FUT"
                    DhanSymbol = "MIDCPNIFTY "+" "+str(ExpiryMonth)+" "+" FUT"
                    AliceSymbol = "MIDCPNIFTY "+str(ExpiryMonth)+" "+" FUT"
                    UpstoxSymbol = "MIDCPNIFTY"+str(ExpiryYear)+str(ExpiryMonth)+"FUT"
                    FyresSymbol = "MIDCPNIFTY"+str(ExpiryYear)+str(ExpiryMonth)+"FUT"
                    MotilalSymbol = "FINNIFTY "+str(ExpiryDate)+"-"+str(ExpiryMonth)+"-"+str(ExpiryYear)
                    ...
                    Buy_Order(str(symbol.replace("'", "")), str(tokenresult[0].replace("'", "")), "VPS_MIDCAP_Future", DhanSymbol, AliceSymbol, UpstoxSymbol, FyresSymbol, MotilalSymbol)

                    trade = "INSERT INTO delta_midcp_fut_orders_1h (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy) values ('"+str(symbol.replace("'", ""))+"', '"+str(tokenresult[0].replace("'", ""))+"', 'BUY', '30', '"+str(Idx_Live)+"', '"+str(Idx_Live)+"', '1', '"+str(datetime.datetime.now())+"', 'EMA_Crossover', '0')"
                ...
                if float(Latest) < float(Supertrend_0_4) and float(Latest) < float(Supertrend_0_8) and float(Latest) < float(ema1) and float(Previous) > float(Supertrend_0_4): #and float(Previous) > float(Supertrend_0_8) and float(Previous) > float(ema1):
                    ...
                    Short_Order(str(symbol.replace("'", "")), str(tokenresult[0].replace("'", "")), "VPS_MIDCAP_Future", DhanSymbol, AliceSymbol, UpstoxSymbol, FyresSymbol, MotilalSymbol)

                    trade = "INSERT INTO delta_midcp_fut_orders_1h (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy) values ('"+str(symbol.replace("'", ""))+"', '"+str(tokenresult[0].replace("'", ""))+"', 'SHORT', '30', '"+str(Idx_Live)+"', '"+str(Idx_Live)+"', '1', '"+str(datetime.datetime.now())+"', 'EMA_Crossover', '0')"
                    mycursor.execute(trade)
                    mydb.commit()
            ...
            if Position == "1":
                ...
                if Type == "BUY":
                    ...
                    if float(Latest) < float(Supertrend_0_4) and float(Latest) < float(Supertrend_0_8) and float(Latest) < float(ema1):
                        ...
                        Sell_Order(Symbol, Token, "VPS_MIDCAP_Future")

                        trade = "INSERT INTO delta_midcp_fut_orders_1h (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('NIFTY_FUTURE', 'None', 'SELL', '30', 'None', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
                ...
                if Type == "SHORT":
                    ...
                    if float(Latest) > float(Supertrend_0_4) and float(Latest) > float(Supertrend_0_8) and float(Latest) > float(ema1):
                        ...
                        Buy_Order(str(symbol.replace("'", "")), str(tokenresult[0].replace("'", "")), "VPS_MIDCAP_Future", DhanSymbol, AliceSymbol, UpstoxSymbol, FyresSymbol)

                        trade = "INSERT INTO delta_midcp_fut_orders_1h (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('NIFTY_FUTURE', 'None', 'SHORTEXIT', '30', 'None', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
