...
def Supertrend_04():
    ...
    df = pd.read_csv('C://Xampp/htdocs/Algo_Trader/CSV_Files/heikin_midcapnifty_Fut_1H.csv')
...
def ema():
    while True:
        ...
        csv_file = 'C://Xampp/htdocs/Algo_Trader/CSV_Files/heikin_midcapnifty_Fut_1H.csv'  # Replace with the path to your CSV file
...
while True:
    ...
    if str(datetime.datetime.now()) < str(data.date())+" 11:15:00.000000":
    ...
    if str(datetime.datetime.now()) > str(data.date())+" 11:15:00.000000":
        ...
        while True:
            ...
            sql = "select * from delta_midcp_opt_orders_1h ORDER BY DateTime DESC"
            ...
            if Position == "0":
                ...
                if float(Latest) > float(Supertrend_0_4) and float(Latest) > float(Supertrend_0_8) and float(Latest) > float(ema1) and float(Previous) < float(Supertrend_0_4):# and float(Previous) < float(Supertrend_0_8) and float(Previous) < float(ema1):
                    symbol = "MIDCPNIFTY"+str(ExpiryDate)+str(ExpiryMonth)+str(ExpiryYear)+str(strikeprice)+"CE"
                    DhanSymbol = "MIDCPNIFTY "+str(ExpiryDate)+" "+str(ExpiryMonth)+" "+str(strikeprice)+" CALL"
                    AliceSymbol = "MIDCPNIFTY "+str(ExpiryDate)+str(ExpiryMonth)+str(ExpiryYear)+"C"+str(strikeprice)
                    UpstoxSymbol = "MIDCPNIFTY"+str(ExpiryYear)+str(ExpiryMonth)+str(strikeprice)+"CE"
                    FyresSymbol = "MIDCPNIFTY"+str(ExpiryYear)+str(ExpiryMonth)+str(strikeprice)+"CE"
                    ...
                    Buy_Order(str(symbol.replace("'", "")), str(tokenresult[0].replace("'", "")), "VPS_MIDCAP_Option", DhanSymbol, AliceSymbol, UpstoxSymbol, FyresSymbol, MotilalSymbol, Zerodha)

                    trade = "INSERT INTO delta_midcp_opt_orders_1h (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('NIFTY_FUTURE', 'None', 'BUY', '30', '0', '"+str(Idx_Live)+"', '1', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                ...
                if float(Latest) < float(Supertrend_0_4) and float(Latest) < float(Supertrend_0_8) and float(Latest) < float(ema1) and float(Previous) > float(Supertrend_0_4): #and float(Previous) > float(Supertrend_0_8) and float(Previous) > float(ema1):
                    symbol = "MIDCPNIFTY"+str(ExpiryDate)+str(ExpiryMonth)+str(ExpiryYear)+str(strikeprice)+"PE"
                    DhanSymbol = "MIDCPNIFTY "+str(ExpiryDate)+" "+str(ExpiryMonth)+" "+str(strikeprice)+" PUT"
                    AliceSymbol = "MIDCPNIFTY "+str(ExpiryDate)+str(ExpiryMonth)+str(ExpiryYear)+"P"+str(strikeprice)
                    UpstoxSymbol = "MIDCPNIFTY"+str(ExpiryYear)+str(ExpiryMonth)+str(strikeprice)+"PE"
                    FyresSymbol = "MIDCPNIFTY"+str(ExpiryYear)+str(ExpiryMonth)+str(strikeprice)+"PE"
                    ...
                    trade = "INSERT INTO delta_midcp_opt_orders_1h (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('"+str(symbol.replace("'", ""))+"', '"+str(tokenresult[0].replace("'", ""))+"', 'BUY', '30', '0', '"+str(Idx_Live)+"', '1', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                    mycursor.execute(trade)
                    mydb.commit()
            ...
            if Position == "1":
                ...
                if Type == "BUY":
                    ...
                    if float(Latest) < float(Supertrend_0_4) and float(Latest) < float(Supertrend_0_8) and float(Latest) < float(ema1):
                        ...
                        Sell_Order('MIDCPNIFTY25SEP23FUT', '44196', "VPS_MIDCAP_Future")

                        trade = "INSERT INTO delta_midcp_opt_orders_1h (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('NIFTY_FUTURE', 'None', 'SELL', '30', 'None', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
                ...
                if Type == "SHORT":
                    if float(Latest) > float(Supertrend_0_4) and float(Latest) > float(Supertrend_0_8) and float(Latest) > float(ema1):
                        ...
                        trade = "INSERT INTO delta_midcp_opt_orders_1h (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('NIFTY_FUTURE', 'None', 'SHORTEXIT', '30', 'None', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
