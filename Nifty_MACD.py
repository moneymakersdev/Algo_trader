...
def MACD_Live():
    ...
    df = pd.read_csv('C://Xampp/htdocs/Algo_Trader/CSV_Files/Nifty_15m.csv')  # Make sure your CSV file has columns like 'Close'
        ...
        while True:
            ...
            if Position == "0":
                ...
                if float(Fast_Line_Live) > float(Slow_Line_Live) and float(Fast_Line_Old) < float(Slow_Line_Old):
                    ...
                    UpstoxSymbol = "NIFTY"+str(ExpiryYear)+str(ExpiryMonth[0])+str(ExpiryDate)+str(strikeprice)+"CE"
                    #UpstoxSymbol = "NIFTY"+str(ExpiryYear)+str(ExpiryMonth)+str(strikeprice)+"CE"   #Monthly Expiry
                    FyresSymbol = "NIFTY"+str(ExpiryYear)+str(ExpiryMonth[0])+str(ExpiryDate)+str(strikeprice)+"CE"
                    MotilalSymbol = "NIFTY "+str(ExpiryDate)+"-"+str(ExpiryMonth)+"-20"+str(ExpiryYear)+" CE "+str(strikeprice)
                    ...
                    SL = float(Idx_Live)-50

                    trade = "INSERT INTO nifty_trades (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('"+str(symbol.replace("'", ""))+"', '"+str(tokenresult.replace("'", ""))+"', 'BUY', '30', '"+str(option_live)+"', '"+str(Idx_Live)+"', '1', '"+str(datetime.datetime.now())+"', 'BNF', '"+str(SL)+"')"
                ...
                if float(Fast_Line_Live) < float(Slow_Line_Live) and float(Fast_Line_Old) > float(Slow_Line_Old):
                    ...
                    UpstoxSymbol = "NIFTY"+str(ExpiryYear)+str(ExpiryMonth[0])+str(ExpiryDate)+str(strikeprice)+"PE"
                    ...
                    MotilalSymbol = "NIFTY "+str(ExpiryDate)+"-"+str(ExpiryMonth)+"-20"+str(ExpiryYear)+" PE "+str(strikeprice)
            ...
            if Position == "1":
                ...
                if "CE" in Symbol:
                    Profit = float(Idx_bought_Price)+50
...
#
                        #trade = "update nifty_trades set SL='"+str(Trailing_SL)+"' where DateTime='"+str(Bought_DateTime)+"'"
                        #mycursor.execute(trade)
                        #mydb.commit()

                        #Sell_Order(Symbol, Token, "BNF")
                        trade = "INSERT INTO nifty_trades (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, '0') values ('"+str(Symbol.replace("'", ""))+"', '"+str(Token.replace("'", ""))+"', 'SELL', '30', '"+str(Option_live)+"', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'BNF', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
                        exit()
                    ...
                    if float(Idx_Live) <= float(Sl):
                        ...
                        #Sell_Order(Symbol, Token, "BNF")
                        trade = "INSERT INTO nifty_trades (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('"+str(Symbol.replace("'", ""))+"', '"+str(Token.replace("'", ""))+"', 'SELL', '30', '"+str(Option_live)+"', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'BNF', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
...
#
                        ...
                        #Sell_Order(Symbol, Token, "BNF")
                        trade = "INSERT INTO nifty_trades (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('"+str(Symbol.replace("'", ""))+"', '"+str(Token.replace("'", ""))+"', 'SL', '30', '"+str(Option_live)+"', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'BNF', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
                        exit()
