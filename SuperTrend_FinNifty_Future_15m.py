...
def Supertrend_04():
    ...
    df = pd.read_csv('C://Xampp/htdocs/Algo_Trader/CSV_Files/heikin_finnifty_Fut_15m.csv')
...
def ema():
    while True:
        ...
        csv_file = 'C://Xampp/htdocs/Algo_Trader/CSV_Files/heikin_finnifty_Fut_15m.csv'  # Replace with the path to your CSV file
...
while True:
    ...
    if str(datetime.datetime.now()) > str(data.date())+" 09:30:00.000000":
        ...
        while True:
            ...
            apikey = "64FFpnmP"
            secretkey = "e6fe94fe-016b-4eb0-8e83-b4b18232b2fd"
            user_id = "VJVG1363"
            password = "2233"
            totp = "WSPHTFHS3AF3YIALCM4EQBAJME"
            ...
            sql = "select * from delta_finnifty_fut_orders_15m ORDER BY DateTime DESC"
            ...
            if Position == "0":
                ...
                if float(Latest) > float(Supertrend_0_4) and float(Latest) > float(Supertrend_0_8) and float(Latest) > float(ema1) and float(Previous) < float(Supertrend_0_4):# and float(Previous) < float(Supertrend_0_8) and float(Previous) < float(ema1):
                    ...
                    #Buy_Order(str(symbol.replace("'", "")), str(tokenresult[0].replace("'", "")), "delta_orders_nifty_15m", DhanSymbol)

                    trade = "INSERT INTO delta_finnifty_fut_orders_15m (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('FINNIFTY_FUTURE', 'None', 'BUY', '30', '0', '"+str(Idx_Live)+"', '1', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                ...
                if float(Latest) < float(Supertrend_0_4) and float(Latest) < float(Supertrend_0_8) and float(Latest) < float(ema1) and float(Previous) > float(Supertrend_0_4): #and float(Previous) > float(Supertrend_0_8) and float(Previous) > float(ema1):
                    ...
                    trade = "INSERT INTO delta_finnifty_fut_orders_15m (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('FINNIFTY_FUTURE', 'None', 'SHORT', '30', '0', '"+str(Idx_Live)+"', '1', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                    mycursor.execute(trade)
                    mydb.commit()
            ...
            if Position == "1":
                ...
                if Type == "BUY":
                    ...
                    if float(Latest) < float(Supertrend_0_4) and float(Latest) < float(Supertrend_0_8) and float(Latest) < float(ema1):
                        ...
                        trade = "INSERT INTO delta_finnifty_fut_orders_15m (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('FINNIFTY_FUTURE', 'None', 'SELL', '30', 'None', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
                ...
                if Type == "SHORT":
                    if float(Latest) > float(Supertrend_0_4) and float(Latest) > float(Supertrend_0_8) and float(Latest) > float(ema1):
                        ...
                        trade = "INSERT INTO delta_finnifty_fut_orders_15m (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('NIFTY_FUTURE', 'None', 'SHORTEXIT', '30', 'None', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
