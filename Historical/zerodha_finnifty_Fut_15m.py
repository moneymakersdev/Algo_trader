...
while True:
    while True:
        ...
        if str(datetime.datetime.now()) >= str(Today)+" 11:30:00.000000":
            print("start")
            symbol_data = getCandels("FinNifty", str(Today), str(Today), KI["15m"])
            print(symbol_data)
            my_df = pd.DataFrame(symbol_data)
            print(my_df)
            my_df.to_csv(r'{}.csv'.format("C://Xampp/htdocs/Algo_Trader/CSV_Files/finnifty_Fut_15m"), mode="a", index=True, header=False)
            ...
            output_file = 'C://Xampp/htdocs/Algo_Trader/CSV_Files/heikin_finnifty_Fut_15m.csv'
            ...
            while True:
                ...
                my_df = pd.DataFrame(symbol_data)
                print(my_df)
                my_df.iloc[[-1]].to_csv(r'{}.csv'.format("C://Xampp/htdocs/Algo_Trader/CSV_Files/finnifty_Fut_15m"), mode="a", index=True, header=False)
