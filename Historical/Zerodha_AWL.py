...
while True:
    while True:
        ...
        if str(datetime.datetime.now()) <= str(Today)+' 10:15:00.000000':
            print('start')
            symbol_data = getCandels('AWL', str(Today), str(Today), KI['60m'])
            print(symbol_data)
            my_df = pd.DataFrame(symbol_data)
            print(my_df)
            #f = open('Historical/Zerodha_AWL.py', 'w')
            #f.close()
            my_df.to_csv(r'{}.csv'.format('C://Xampp/htdocs/Algo_Trader/CSV_Files/Zerodha_AWL'), mode='a', index=True, header=False)
            ...
            while True:
                ...
                my_df = pd.DataFrame(symbol_data)
                print(my_df)
                my_df.iloc[[-1]].to_csv(r'{}.csv'.format('C://Xampp/htdocs/Algo_Trader/CSV_Files/Zerodha_AWL'), mode='a', index=True, header=False)
