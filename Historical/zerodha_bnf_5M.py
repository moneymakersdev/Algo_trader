...
while True:
    while True:
        ...
        if str(datetime.datetime.now()) >= str(Today)+" 09:20:02.000000":
            ...
            symbol_data = td_app.get_historic_data("NIFTY BANK", bar_size="5 mins")
            my_df = pd.DataFrame(symbol_data)
            print(my_df)
            my_df.to_csv(r'{}.csv'.format("C://Xampp/htdocs/Algo_Trader/CSV_Files/bnf_5m"), mode="a", index=True, header=False)
            ...
            while True:
                ...
                my_df = pd.DataFrame(symbol_data)
                print(my_df)
                my_df.iloc[[-1]].to_csv(r'{}.csv'.format("C://Xampp/htdocs/Algo_Trader/CSV_Files/bnf_5m"), mode="a", index=True, header=False)
