...
#def calculate_heikin_ashi(df):
#    
#    return ta.ha(my_df["o"], my_df["h"], my_df['l'], my_df['c'])
...
while True:
    while True:
        ...
        if str(datetime.datetime.now()) >= str(Today)+" 09:16:02.000000":
            ...
            symbol_data = td_app.get_historic_data("NIFTY BANK", bar_size="1 min")

            my_df = pd.DataFrame(symbol_data)
            print(my_df)
            my_df.to_csv(r'{}.csv'.format("C://Xampp/htdocs/Algo_Trader/CSV_Files/bnf_1m"), mode="a", index=True, header=False)
            ...
            while True:
                ...
                my_df = pd.DataFrame(symbol_data)
                print(my_df)
                my_df.iloc[[-1]].to_csv(r'{}.csv'.format("C://Xampp/htdocs/Algo_Trader/CSV_Files/bnf_1m"), mode="a", index=True, header=False)
