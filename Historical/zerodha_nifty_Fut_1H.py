...
while True:
    while True:
        if str(datetime.datetime.now()) <= str(Today)+" 12:15:00.000000":
            print("Market is Closed")
            sleep(1)
            break
        if str(datetime.datetime.now()) >= str(Today)+" 12:15:00.000000":
            print("start")
            #symbol_data = getCandels("NIFTY", str(Today), str(Today), KI["60m"])
            #print(symbol_data)
            ...
            symbol_data = td_app.get_historic_data("NIFTY-I", bar_size="60 mins")

            my_df = pd.DataFrame(symbol_data)
            print(my_df)
            my_df.to_csv(r'{}.csv'.format("C://Xampp/htdocs/Algo_Trader/CSV_Files/nifty_Fut_1H"), mode="a", index=True, header=False)
            ...
            output_file = 'C://Xampp/htdocs/Algo_Trader/CSV_Files/heikin_nifty_Fut_1H.csv'
            ...
            while True:
                ...
                my_df = pd.DataFrame(symbol_data)
                print(my_df)
                my_df.iloc[[-1]].to_csv(r'{}.csv'.format("C://Xampp/htdocs/Algo_Trader/CSV_Files/nifty_Fut_1H"), mode="a", index=True, header=False)
