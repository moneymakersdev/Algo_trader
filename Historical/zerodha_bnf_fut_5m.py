...
while True:
    while True:
        if str(datetime.datetime.now()) <= str(Today)+" 09:20:02.000000":
            print("Market is Closed")
            sleep(1)
            break
        if str(datetime.datetime.now()) >= str(Today)+" 09:20:02.000000":
            print("start")
            #symbol_data = getCandels("BANKNIFTY", str(Today), str(Today), KI["5m"])
            #print(symbol_data)
            ...
            symbol_data = td_app.get_historic_data("BANKNIFTY-I", bar_size="5 mins")

            my_df = pd.DataFrame(symbol_data)
            print(my_df)
            my_df.to_csv(r'{}.csv'.format("C://Xampp/htdocs/Algo_Trader/CSV_Files/bnf_Fut_5m"), mode="a", index=True, header=False)
            ...
            output_file = 'C://Xampp/htdocs/Algo_Trader/CSV_Files/heikin_bnf_Fut_5m.csv'
            ...
            while True:
                ...
                my_df.iloc[[-1]].to_csv(r'{}.csv'.format("C://Xampp/htdocs/Algo_Trader/CSV_Files/bnf_Fut_5m"), mode="a", index=True, header=False)
