...
while True:
    ...
    if str(datetime.datetime.now()) > str(data.date())+" 09:30:00.000000":
        ...
        while True:
            ...
            apikey = "0c8vNXam"
            secretkey = "acf4eb5b-a735-4633-8b77-e0baf6da6bd9"
            user_id = "G62491"
            password = "8199"
            totp = "CMXKMHFEBGMYSHYIPX5B5O7ONI"
            ...
            sql = "select * from delta_nifty_fut_orders_1h ORDER BY DateTime DESC"
            ...
            if Position == "0":
                ...
                if float(Latest) > float(Supertrend_0_4) and float(Latest) > float(Supertrend_0_8) and float(Latest) > float(ema1) and float(Previous) < float(Supertrend_0_4):# and float(Previous) < float(Supertrend_0_8) and float(Previous) < float(ema1):
                    ...
                    MotilalSymbol = "NIFTY "+str(ExpiryDate)+"-"+str(ExpiryMonth)+"-"+str(ExpiryYear)

                    token = 'select token from instruments where symbol="'+symbol+'"'
                    mycursor.execute(token)
                    tokenresult = mycursor.fetchone()

                    Buy_Order(str(symbol.replace("'", "")), str(tokenresult[0].replace("'", "")), "VPS_NIFTY_Future", DhanSymbol, AliceSymbol, UpstoxSymbol, FyresSymbol, MotilalSymbol)

                    trade = "INSERT INTO delta_nifty_fut_orders_1h (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('NIFTY_FUTURE', 'None', 'BUY', '30', '0', '"+str(Idx_Live)+"', '1', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                ...
                if float(Latest) < float(Supertrend_0_4) and float(Latest) < float(Supertrend_0_8) and float(Latest) < float(ema1) and float(Previous) > float(Supertrend_0_4): #and float(Previous) > float(Supertrend_0_8) and float(Previous) > float(ema1):
                    ...
                    Short_Order(str(symbol.replace("'", "")), str(tokenresult[0].replace("'", "")), "VPS_NIFTY_Future", DhanSymbol, AliceSymbol, UpstoxSymbol, FyresSymbol, MotilalSymbol)

                    trade = "INSERT INTO delta_nifty_fut_orders_1h (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('NIFTY_FUTURE', 'None', 'SHORT', '30', '0', '"+str(Idx_Live)+"', '1', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                    mycursor.execute(trade)
                    mydb.commit()
            ...
            if Position == "1":
                ...
                if Type == "BUY":
                    ...
                    if float(Latest) < float(Supertrend_0_4) and float(Latest) < float(Supertrend_0_8) and float(Latest) < float(ema1):
                        ...
                        Sell_Order(Symbol, Token, "VPS_NIFTY_Future")

                        trade = "INSERT INTO delta_nifty_fut_orders_1h (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('NIFTY_FUTURE', 'None', 'SELL', '30', 'None', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
                ...
                if Type == "SHORT":
                    ...
                    if float(Latest) > float(Supertrend_0_4) and float(Latest) > float(Supertrend_0_8) and float(Latest) > float(ema1):
                        ...
                        trade = "INSERT INTO delta_nifty_fut_orders_1h (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('NIFTY_FUTURE', 'None', 'SHORTEXIT', '30', 'None', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
