...
def Supertrend_04():
    ...
    df = pd.read_csv('C://Xampp/htdocs/Algo_Trader/CSV_Files/heikin_bnf_Fut_5m.csv')
...
def ema():
    while True:
        ...
        csv_file = 'C://Xampp/htdocs/Algo_Trader/CSV_Files/heikin_bnf_Fut_5m.csv'  # Replace with the path to your CSV file
...
while True:
    ...
    if str(datetime.datetime.now()) > str(data.date())+" 09:30:00.000000":
        ...
        while True:
            print("5 min")
            apikey = "NIQ2NnMK"
            secretkey = "dc67c7b4-03a9-4aff-88ab-9db2780b2d56"
            user_id = "VJVG1040"
            password = "1234"
            totp = "Z53B3GED4KJHSF2PVG4RKCFQRM"
            ...
            sql = "select * from delta_bnf_fut_orders_5m ORDER BY DateTime DESC"
            ...
            if Position == "0":
                ...
                if float(Latest) > float(Supertrend_0_4) and float(Latest) > float(Supertrend_0_8) and float(Latest) > float(ema1) and float(Previous) < float(Supertrend_0_4):# and float(Previous) < float(Supertrend_0_8) and float(Previous) < float(ema1):
                    ...
                    #Buy_Order(str(symbol.replace("'", "")), str(tokenresult[0].replace("'", "")), "delta_orders_5m", DhanSymbol)

                    trade = "INSERT INTO delta_bnf_fut_orders_5m (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('BANKNIFTY_FUTURE', 'None', 'BUY', '15', '0', '"+str(Idx_Live)+"', '1', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                ...
                if float(Latest) < float(Supertrend_0_4) and float(Latest) < float(Supertrend_0_8) and float(Latest) < float(ema1) and float(Previous) > float(Supertrend_0_4): #and float(Previous) > float(Supertrend_0_8) and float(Previous) > float(ema1):
                    ...
                    trade = "INSERT INTO delta_bnf_fut_orders_5m (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('BANKNIFTY_FUTURE', 'None', 'SHORT', '30', '0', '"+str(Idx_Live)+"', '1', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                    mycursor.execute(trade)
                    mydb.commit()
            ...
            if Position == "1":
                ...
                if Type == "BUY":
                    if float(Latest) < float(Supertrend_0_4) and float(Latest) < float(Supertrend_0_8) and float(Latest) < float(ema1):
                        ...
                        trade = "INSERT INTO delta_bnf_fut_orders_5m (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('BANKNIFTY_FUTURE', 'None', 'SELL', '30', 'None', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
                ...
                if Type == "SHORT":
                    if float(Latest) > float(Supertrend_0_4) and float(Latest) > float(Supertrend_0_8) and float(Latest) > float(ema1):
                        ...
                        #Sell_Order(Symbol, Token, "Delta")
                        trade = "INSERT INTO delta_bnf_fut_orders_5m (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('BANKNIFTY_FUTURE', 'None', 'SHORTEXIT', '30', 'None', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
