...
#def TA_SuperTrend():
#    df = pd.read_csv('C://Xampp/htdocs/Algo_Trader/CSV_Files/heikin_bnf_Fut_1m.csv')
#    
#    # Input parameters
#    atr_period = 10
#    factor = 1
...
while True:
    ...
    if str(datetime.datetime.now()) > str(data.date())+" 09:17:00.000000":
        ...
        while True:
            apikey = "qG8zMMHN"
            secretkey = "e8d62cd3-0425-4d54-85e0-8451012e1960"
            user_id = "M51346381"
            password = "1007"
            totp = "35N6PALC4QYRFGF3WJIVO53SFE"
            ...
            supertrend = open('scalper_master.txt', 'a')
            supertrend.write(filedata)
            supertrend.close()

            print(datetime.datetime.now())

            sql = "select * from scalper_master ORDER BY DateTime DESC"
            ...
            if Position == "0":
                ...
                if float(Latest) > float(TA_SuperTrend_1) and float(Previous) < float(TA_SuperTrend_1):
                    ...
                    #Buy_Order(str(symbol.replace("'", "")), str(tokenresult.replace("'", "")), "scalper_master", DhanSymbol, AliceSymbol, UpstoxSymbol, FyresSymbol, MotilalSymbol, Zerodha, str(round(float(strikeprice)-100)), "CALL", IciciExpiry)

                    trade = "INSERT INTO scalper_master (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('"+str(symbol.replace("'", ""))+"', '"+str(tokenresult.replace("'", ""))+"', 'BUY', '15', '"+str(option_live)+"', '"+str(Open)+"', '1', '"+str(datetime.datetime.now())+"', 'scalper_master', '0')"
                ...
                if float(Latest) < float(TA_SuperTrend_1) and float(Previous) > float(TA_SuperTrend_1):
                    ...
                    #Buy_Order(str(symbol.replace("'", "")), str(tokenresult.replace("'", "")), "scalper_master", DhanSymbol, AliceSymbol, UpstoxSymbol, FyresSymbol, MotilalSymbol, Zerodha, str(round(float(strikeprice)+100)), "PUT", IciciExpiry)
            ...
            if Position == "1":
                ...
                P_L_Loss = float(Bough_Idx)-(float(Bough_Idx)*0.07/100)
                ...
                if str(datetime.datetime.now()) >= str(data.date())+" 15:10:00.000000":
                    #Sell_Order(Symbol, Token, "scalper_master")

                    trade = "INSERT INTO scalper_master (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('"+str(Symbol.replace("'", ""))+"', '"+str(Token.replace("'", ""))+"', 'SELL', '15', '"+str(Option_live)+"', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'scalper_master', '0')"
                    mycursor.execute(trade)
                    mydb.commit()
                ...
                if "CE" in str(Symbol):
                    ...
                    if float(Idx_Live) <= float(P_L_Loss):
                        ...
                        trade = "INSERT INTO scalper_master (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('"+str(Symbol.replace("'", ""))+"', '"+str(Token.replace("'", ""))+"', 'SL', '15', '"+str(Option_live)+"', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'scalper_master', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
