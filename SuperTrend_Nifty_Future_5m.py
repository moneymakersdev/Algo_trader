...
def Supertrend_04():
    ...
    df = pd.read_csv('C://Xampp/htdocs/Algo_Trader/CSV_Files/heikin_nifty_Fut_5m.csv')
...
def ema():
    while True:
        ...
        csv_file = 'C://Xampp/htdocs/Algo_Trader/CSV_Files/heikin_nifty_Fut_5m.csv'  # Replace with the path to your CSV file
...
while True:
    ...
    if str(datetime.datetime.now()) > str(data.date())+" 09:30:00.000000":
        ...
        while True:
            ...
            sql = "select * from delta_nifty_fut_orders_5m ORDER BY DateTime DESC"
            ...
            if Position == "0":
                ...
                if float(Latest) > float(Supertrend_0_4) and float(Latest) > float(Supertrend_0_8) and float(Latest) > float(ema1) and float(Previous) < float(Supertrend_0_4):# and float(Previous) < float(Supertrend_0_8) and float(Previous) < float(ema1):
                    ...
                    DhanSymbol = "NIFTY "+" "+str(ExpiryMonth)+" "+" FUT"
                    AliceSymbol = "NIFTY "+str(ExpiryMonth)+" "+" FUT"
                    UpstoxSymbol = "NIFTY"+str(ExpiryYear)+str(ExpiryMonth)+"FUT"
                    FyresSymbol = "NIFTY"+str(ExpiryYear)+str(ExpiryMonth)+"FUT"

                    token = 'select token from instruments where symbol="'+symbol+'"'
                    mycursor.execute(token)
                    tokenresult = mycursor.fetchone()
                    
                    Buy_Order(str(symbol.replace("'", "")), str(tokenresult[0].replace("'", "")), "delta_orders_nifty_5m", DhanSymbol, AliceSymbol, UpstoxSymbol, FyresSymbol)

                    trade = "INSERT INTO delta_nifty_fut_orders_5m (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('NIFTY_FUTURE', 'None', 'BUY', '30', '0', '"+str(Idx_Live)+"', '1', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                ...
                if float(Latest) < float(Supertrend_0_4) and float(Latest) < float(Supertrend_0_8) and float(Latest) < float(ema1) and float(Previous) > float(Supertrend_0_4): #and float(Previous) > float(Supertrend_0_8) and float(Previous) > float(ema1):
                    ...
                    trade = "INSERT INTO delta_nifty_fut_orders_5m (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('NIFTY_FUTURE', 'None', 'SHORT', '30', '0', '"+str(Idx_Live)+"', '1', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                    mycursor.execute(trade)
                    mydb.commit()
            ...
            if Position == "1":
                ...
                if Type == "BUY":
                    if float(Latest) < float(Supertrend_0_4) and float(Latest) < float(Supertrend_0_8) and float(Latest) < float(ema1):
                        ...
                        trade = "INSERT INTO delta_nifty_fut_orders_5m (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('NIFTY_FUTURE', 'None', 'SELL', '30', 'None', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
                    ...
                    if str(datetime.datetime.now()) >= str(data.date())+" 15:10:00.000000":
                        ...
                        trade = "INSERT INTO delta_nifty_fut_orders_5m (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('NIFTY_FUTURE', 'None', 'SHORT EXIT', '30', 'None', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
                ...
                if Type == "SHORT":
                    if float(Latest) > float(Supertrend_0_4) and float(Latest) > float(Supertrend_0_8) and float(Latest) > float(ema1):
                        ...
                        trade = "INSERT INTO delta_nifty_fut_orders_5m (Name, Token, Type, Qty, Price, IdxPrice, Position, DateTime, Strategy, SL) values ('NIFTY_FUTURE', 'None', 'SHORTEXIT', '30', 'None', '"+str(Idx_Live)+"', '0', '"+str(datetime.datetime.now())+"', 'Delta', '0')"
                        mycursor.execute(trade)
                        mydb.commit()
